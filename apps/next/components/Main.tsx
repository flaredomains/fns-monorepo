  ...
  return (
    <div className='bg-[#0F172A] min-h-screen w-screen relative overflow-hidden'>
      ...
      <div className='absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2'>
        ...
        <form onSubmit={handleSubmit} className='flex items-center w-full mt-10 py-2 px-4 h-12 bg-gray-700 border-2 border-gray-500'>
          <Image className='z-10 h-6 w-6 mr-2' src={Search} alt="FNS" />
          <input type="text" onChange={(e)=>{setRoute(e.target.value)}} className='w-full bg-transparent font-normal text-base text-white focus:outline-none placeholder:text-gray-300 placeholder:font-normal' placeholder='Search New Names or Addresses'/>
        </form>
      </div>
    </div>
  )
